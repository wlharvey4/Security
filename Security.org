...
#+author:Pinecone062
#+date:April 14, 2019
#+macro:version 0.2.12.<2019-05-01 Wed 13:24>
...
app.get("/resource/secret", jwtCheck, (req, res) => {
    res
    .status(200)
    .send("Secret resource; you must be logged in to see this.");
});
...
#+CINDEX:jumbotron
#+CINDEX:AJAX calls
00:36 Let's take a look at the front-end that was pre-built for us.  There was
a button labeled =public=, and another labeled =private=.  They will both
update the jumbotron beneath it with the response that we're getting from our
AJAX calls.
...
are getting from the server.  For the =UI_updates=, the helper functions are
already created.  We will focus on the actual logic in here.
...
01:03 For now, none of the buttons do anything.  Let's go back to our code.
=headlineBtn= here is the button labeled =public=.  We will start by doing a
~fetch~ to our public ~/resource~, which is on the API server at the
~/resource~ endpoint.
...
authenticate.  Let's go the =login= button click event listener and do a =POST=
request to our authorization server.
...
will also specify a few options to our request.  First, this is not a =GET=, so
we will need to add method =POST=; then we need to specify the headers to tell
our server that we are sending =Content-Type:= "application/json".
