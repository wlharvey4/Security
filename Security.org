...
#+author:Pinecone062
#+date:May 11, 2019
#+macro:version 0.6.0.<2019-05-11 Sat 10:42>
...
a file server.  Feel free to change this to your choice.
...
# http-server defaults to /public and port 8080
STATIC_SERVER := http-server
STATIC_SERVER_URL := $(URL)
STATIC_SERVER_PORT := 8080
START_STATIC_SERVER := ./node_modules/.bin/$(STATIC_SERVER)
...
URL_PORT         := $(URL):$(PORT)
STATIC_SERVER_URL_PORT := $(STATIC_SERVER_URL):$(STATIC_SERVER_PORT)

OPEN_BROWSER     := $(OPEN) $(BROWSER)
OPEN_BROWSER_URI  = $(OPEN_BROWSER) $(URL_PORT)/$(ROUTE)
OPEN_FRONT_END   := $(OPEN) $(FRONT_END)
OPEN_FRONT_END_URI := $(OPEN_FRONT_END) $(STATIC_SERVER_URL_PORT)
...
terminate :
	...
	@pgrep -lf $(STATIC_SERVER)
	@pkill -lf $(STATIC_SERVER)
#+end_src
...
Then we'll make a controller called =Main Controller as View Model=
(=MainCtrl_vm=).
...
#+caption[angularjs-auth-jwt-index.html]:Code for the AngularJS Auth Jwt @code{index.html} file
#+name:angularjs-auth-jwt-index.html
#+begin_src html :tangle angularjs-auth-jwt/public/index.html :mkdirp yes :noweb yes
<!DOCTYPE html>
...
file called ~app.js~ and we'll put this in an immediately invoking function. We'll
have our app have no dependencies.

{{{heading(Create ~app.js~ Application File)}}}
...
{{{subheading(Create the App Controller =MainCtrl=)}}}

01:40 We're going to create an =app.controller=.  We'll call this Main
Controller (=MainCtrl=).

#+name:angularjs-auth-jwt-app--controller
#+begin_src js :noweb yes
...
{{{subheading(Create the Random User Factory)}}}

02:09 Now let's create this =RandomUserFactory=.  This will have the =getUser=.
This will be a simple HTTP =GET=.  We'll use our =API_URL/randomUser= and we'll
return that.
...
#+name:angularjs-auth-jwt-app.js
#+begin_src js :tangle angularjs-auth-jwt/public/app.js :noweb yes :mkdirp yes
(function() {
    'use strict';
    <<angularjs-auth-jwt-app--var_app>>
    <<angularjs-auth-jwt-app--const_API_URL>>
    <<angularjs-auth-jwt-app--controller>>
    <<angularjs-auth-jwt-app---RandomUserFactory>>
})()
#+end_src

**** Add CORS Module to ~server.js~
...
#+caption:server2.js
#+name:angularjs-auth-jwt-server.js
#+begin_src js :tangle angularjs-auth-jwt/server2.js :mkdirp yes :noweb yes
...
server2 : check-server
	$(CURL_URI_PP)
	@$(OPEN_BROWSER_URI)
#+end_src

***** Makefile for Static File Server
