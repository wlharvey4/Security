...
#+author:Pinecone062
#+date:May 9, 2019
#+macro:version 0.5.5.<2019-05-09 Thu 15:36>
...
***** Makefile
Hit the server endpoint at =/random-user= with a =GET= request:
: make server1

The Makefile checks for a running server; if it is not running, then
it starts the server before hitting the endpoint with a =GET= request.
...
.phony : check-server
check-server :
	@pgrep -qlf 'node server[[:digit:]]' || { node server1.js & sleep 2; }
#	@curl -sI localhost:3000/random-user | grep -q 200 || { node server1.js & sleep 2; }

.phony : server1
server1 : check-server
	@curl -sS localhost:3000/random-user | json_xs
	@open -a "Google Chrome" "http://localhost:3000/random-user"
