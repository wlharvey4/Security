...
#+author:Pinecone062
#+date:April 14, 2019
#+macro:version Version 0.2.0.<2019-04-29 Mon 17:06>
By {{{author}}}
{{{version}}}
* JSON Web Tokens
...
** Course---JSON Weg Token Authentication with Node.js
:PROPERTIES:
:source:   Egghead.io
:author:   Joel Lord
:END:
...
~lesson#~ directories for each lesson 1 through 8.  The following code creates
the root directory and initializes a Node package within it.

#+name:directory-setup
#+begin_src sh :results output append :exports both
[ -d egghead-jwt-auth ] && echo "Root directory has already been created" || mkdir -v egghead-jwt-auth
cd egghead-jwt-auth
[ -e package.json ] && echo "package.json has already been created" || yarn --yes init
#+end_src
...
01:53 If we go back to our terminal, we can start the server by using Node.
: node run
...
us a =404= error.  That's it.  You've got your first Express server up and
running.
